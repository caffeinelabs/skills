...
/// of packed posting arrays, and nothing else. It stores NO function values, so it is `stable`
...
module {
  ...
  public func put<K, V>(
    ...
    SecondaryIndex.onChange<K>(self.ix, compare, k, old.map<V, Row>(_toRow), ?(_toRow(v)));
  };
  ...
  public func delete<K, V>(
    ...
    SecondaryIndex.onChange<K>(self.ix, compare, k, old.map<V, Row>(_toRow), null);
  };
  ...
  func materialize<K, V>(self : IndexedMap<K, V>, compare : (K, K) -> Order.Order, refs : Iter.Iter<K>) : Iter.Iter<(K, V)> =
    refs.filterMap(func (k : K) : ?(K, V) =
      (self.inner.get(compare, k)).map<V, (K, V)>(func (v : V) : (K, V) = (k, v)));
  ...
  public func entity<K, V>(
    ...
    base.withServed<V>(func (toPredRow : V -> Predicate.Row) : Entity.Served {
      ...
      func rows(refs : Iter.Iter<K>) : Iter.Iter<Predicate.Row> =
        refs.filterMap(func (k : K) : ?Predicate.Row =
          (self.inner.get(compare, k)).map<V, Predicate.Row>(toPredRow));
      {
        ...
        // No zone maps on the heap store — the executor uses the ordinary scan.
        prune = null;
        ...
        stats = ?{
          ...
          countUpTo  = func (col : Text, v : Value, _limit : Nat) : ?Entity.CountEstimate =
            switch (SecondaryIndex.count(self.ix, col, v)) {
              case (?n) ?#exact(n);
              case null null;
            };
          ...
          sum        = func (_ : Text) : ?Value = null;   // heap keeps no running sum → scan-fold
          avg        = func (_ : Text) : ?Value = null;   // heap keeps no running sum/count → scan-fold
          // min/max here walk the index in ITERATION order, which is value order
          // only for `#ordered` — that case is served by the planner's own gate.
          extremesExact = func (_ : Text) : Bool = false;
        };
      }
    });
  };
