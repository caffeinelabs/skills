...
///
/// The store is `column -> (Value -> Posting<Ref>)`: one ordered `Map` per declared
...
///
/// A posting is a sorted packed array of refs (`Posting`), iterated in `Ref`
/// order — which is what lets the executor's stable sort resolve orderBy ties
/// exactly as a scan (whose rows arrive in that same key order) does. Adding or
/// removing a ref at either end of that order is `O(1)`; in the middle it costs
/// `O(k)` in the shorter side. `Posting` documents why an array beats a tree here.
///
...
/// is `columns -> ([Value] -> Posting<Ref>)`, one ordered `Map` per declared
...
module {
  ...
  type ColIndex<Ref> = Map.Map<Value, Posting.Posting<Ref>>;
  type SeqIndex<Ref> = Map.Map<[Value], Posting.Posting<Ref>>;
  ...
  public func emptyWith<Ref>(decls : [(Text, Kind)], composites : [([Text], Kind)]) : Index<Ref> {
    ...
    for ((col, _) in decls.values()) { byCol.add(Text.compare, col, Map.empty<Value, Posting.Posting<Ref>>()) };
    let byCols = Map.empty<[Text], SeqIndex<Ref>>();
    for ((cols, _) in composites.values()) { byCols.add(compareCols, cols, Map.empty<[Value], Posting.Posting<Ref>>()) };
  ...
  public func addDecl<Ref>(ix : Index<Ref>, target : Target, kind : Kind) {
    switch target {
      case (#col col) {
        ...
        ix.byCol.add(Text.compare, col, Map.empty<Value, Posting.Posting<Ref>>());
      ...
      case (#cols cols) {
        ...
        ix.byCols.add(compareCols, cols, Map.empty<[Value], Posting.Posting<Ref>>());
  ...
  func addRef<K, Ref>(ci : Map.Map<K, Posting.Posting<Ref>>, keyCmp : (K, K) -> Order.Order, refCmp : (Ref, Ref) -> Order.Order, key : K, ref : Ref) {
    switch (ci.get(keyCmp, key)) {
      case (?s) { s.add(refCmp, ref) };
      case null { let s = Posting.empty<Ref>(); s.add(refCmp, ref); ci.add(keyCmp, key, s) };
    };
  };

  func removeRef<K, Ref>(ci : Map.Map<K, Posting.Posting<Ref>>, keyCmp : (K, K) -> Order.Order, refCmp : (Ref, Ref) -> Order.Order, key : K, ref : Ref) {
    switch (ci.get(keyCmp, key)) {
      case (?s) { ignore s.remove(refCmp, ref); if (s.size() == 0) ignore ci.delete(keyCmp, key) };
      case null {};
    };
  };

  /// Reflect one mutation of `ref` in every declared index. `refCmp` orders the
  /// postings (the caller passes the already-resolved `Ref` comparator — no
  ...
  public func onChange<Ref>(ix : Index<Ref>, refCmp : (Ref, Ref) -> Order.Order, ref : Ref, oldRow : ?Row, newRow : ?Row) {
    for ((col, _) in ix.decls.values()) {
      ...
      if (not sameKey(oldKey, newKey)) {
        let ci = switch (ix.byCol.get(Text.compare, col)) { case (?c) c; case null { let c = Map.empty<Value, Posting.Posting<Ref>>(); ix.byCol.add(Text.compare, col, c); c } };
    ...
    for ((cols, _) in ix.decls2.values()) {
      ...
      if (not sameSeq(oldKey, newKey)) {
        let ci = switch (ix.byCols.get(compareCols, cols)) { case (?c) c; case null { let c = Map.empty<[Value], Posting.Posting<Ref>>(); ix.byCols.add(compareCols, cols, c); c } };
  ...
  /// posting dedups adds, so a row a live `onChange` already indexed lands
  ...
  /// Backfill, run-wise: a per-row writer for the single-column decl `col`,
  /// taking the key and ref directly — where `indexOne` builds a row and
  /// resolves the column by name, per row. The column's store is resolved
  /// once per writer, and the last key's posting is memoized, so a run of
  /// rows sharing a key costs one map descent for the whole run. Idempotent
  /// exactly as `indexOne` (the posting dedups adds). An undeclared column
  /// yields a no-op writer.
  public func runWriter<Ref>(ix : Index<Ref>, refCmp : (Ref, Ref) -> Order.Order, col : Text) : (Value, Ref) -> () =
    switch (ix.byCol.get(Text.compare, col)) {
      case (?ci) memoWriter(ci, Predicate.compare, refCmp);
      case null func (_key : Value, _ref : Ref) {};
    };

  /// `runWriter` for a composite decl: the key is the row's column-value
  /// vector, in decl (KEY) order.
  public func runWriterSeq<Ref>(ix : Index<Ref>, refCmp : (Ref, Ref) -> Order.Order, cols : [Text]) : ([Value], Ref) -> () =
    switch (ix.byCols.get(compareCols, cols)) {
      case (?ci) memoWriter(ci, compareSeq, refCmp);
      case null func (_key : [Value], _ref : Ref) {};
    };

  func memoWriter<K, Ref>(ci : Map.Map<K, Posting.Posting<Ref>>, keyCmp : (K, K) -> Order.Order, refCmp : (Ref, Ref) -> Order.Order) : (K, Ref) -> () {
    var memo : ?(K, Posting.Posting<Ref>) = null;
    func (key : K, ref : Ref) {
      let s = switch memo {
        case (?(k, s)) if (keyCmp(k, key) == #equal) s else fetch(ci, keyCmp, key);
        case null fetch(ci, keyCmp, key);
      };
      memo := ?(key, s);
      s.add(refCmp, ref);
    };
  ...
  // The posting under `key`, created empty on first touch (as `addRef` does).
  func fetch<K, Ref>(ci : Map.Map<K, Posting.Posting<Ref>>, keyCmp : (K, K) -> Order.Order, key : K) : Posting.Posting<Ref> {
    switch (ci.get(keyCmp, key)) {
      case (?s) s;
      case null { let s = Posting.empty<Ref>(); ci.add(keyCmp, key, s); s };
    };
  };
  ...
  /// row surfaces once). Distinct keys map to disjoint postings.
  ...
  public func range<Ref>(ix : Index<Ref>, col : Text, lo : ?Value, hi : ?Value, dir : { #asc; #desc }) : Iter.Iter<Ref> =
    switch (colOf(ix, col)) {
      case null { emptyIter() };
      case (?ci) {
        let entries : Iter.Iter<(Value, Posting.Posting<Ref>)> = switch dir {
        ...
        let bounded = entries.takeWhile(func ((k, _) : (Value, Posting.Posting<Ref>)) : Bool =
          switch dir {
        ...
        bounded.flatMap(func ((_, s) : (Value, Posting.Posting<Ref>)) : Iter.Iter<Ref> = s.values());
      };
    };
  ...
  /// Heap postings expose their size directly, so the bounded planner count is
  /// O(1): exact through `limit`, otherwise the single sentinel `limit + 1`.
  public func countUpTo<Ref>(ix : Index<Ref>, col : Text, v : Value, limit : Nat) : ?Nat =
    switch (count(ix, col, v)) {
      case (?n) { ?(if (n <= limit) n else limit + 1) };
      case null null;
    };
  ...
  func firstNonNull<Ref>(it : Iter.Iter<(Value, Posting.Posting<Ref>)>) : ?Value {
  ...
  public func compositeRange<Ref>(ix : Index<Ref>, cols : [Text], prefixEqs : [Value], lo : ?Value, hi : ?Value, dir : { #asc; #desc }) : Iter.Iter<Ref> =
    switch (colsOf(ix, cols)) {
      ...
      case (?ci) {
        ...
        let bounded = entries.takeWhile(func ((key, _) : ([Value], Posting.Posting<Ref>)) : Bool {
          var i = 0;
          while (i < k) {
