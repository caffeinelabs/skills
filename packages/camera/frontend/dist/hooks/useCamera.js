...
export const useCamera = (config = {}) => {
    ...
    const createMediaStream = useCallback(async (facing) => {
        try {
            const constraints = {
                video: {
                    facingMode: facing,
    ...
    const setupVideo = useCallback(async (stream) => {
        if (!videoRef.current)
            return false;
    ...
    const switchCamera = useCallback(async (newFacingMode) => {
        ...
        const targetFacingMode = newFacingMode ||
