...
void describe("resolveBlobContentType", () => {
    void it("returns hint when provided", async () => {
        assert.equal(await resolveBlobContentType(JPEG_BYTES, "image/png"), "image/png");
    });
    void it("trims whitespace from hint", async () => {
        assert.equal(await resolveBlobContentType(JPEG_BYTES, "  image/png  "), "image/png");
    });
    void it("sniffs bytes when hint is application/octet-stream", async () => {
        assert.equal(await resolveBlobContentType(JPEG_BYTES, "application/octet-stream"), "image/jpeg");
    });
    ...
    void it("falls back to application/octet-stream for unrecognised bytes", async () => {
        assert.equal(await resolveBlobContentType(UNKNOWN_BYTES), "application/octet-stream");
    });
    void it("falls back to application/octet-stream for empty bytes", async () => {
        assert.equal(await resolveBlobContentType(EMPTY_BYTES), "application/octet-stream");
    });
});
//# sourceMappingURL=resolveBlobContentType.test.js.map